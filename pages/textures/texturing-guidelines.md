...
<li id="bugs-in-textures"><b>If a texture has issues noted as a bug on the Mojira website, Mojang's offical Bug tracker, then changes should be made to fix them if found. This allows for the pack to have these issues in textures already resolved when they are properly added into the base game, saving time. Textures such as unused/misaligned pixels or textures, Jappafication errors, or parity issues are examples of bugs that we try to look for. 
