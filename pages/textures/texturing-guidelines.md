...
<li id="bugs-in-textures"><b>If a texture has issues noted as a bug on the Mojira website, Mojang's offical Bug tracker, then changes should be made to fix them. Some examples of bugs we allow include, but are not limited to, unused/misaligned pixels or textures, Jappafication errors, or parity issues. Fixing bugs allows for the pack to have issues in textures resolved before they are added into the base game which saves us time.  
