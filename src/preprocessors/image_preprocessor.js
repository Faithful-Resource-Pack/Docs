...
function willProcess(nodeSrc, prepath) {
  return {
...
async function getProcessingPathsForNode(node) {
  ...
  if (IS_EXTERNAL.test(value.data)) {
    ...
    domainSlash = value.data;
  }

  const fullPath = path.resolve(options.publicDir, removedDomainSlash);

  if (fs.existsSync(fullPath)) {
    return willProcess(removedDomainSlash, domainSlash);
  } else {
...
/**
 * Replaces Image attributes
 * @param {import("svelte/types/compiler/interfaces").Element} edited 
 * @param {import("svelte/types/compiler/interfaces").Element} node 
 * @returns Replace content
 */
async function replaceInComponent(edited, node) {
  ...
  const [size, sizes] = await createSizes(paths);
  ...
  replaced = insert(
    replaced.content,
    getSrcset(sizes).replaceAll('g/', '/g/'),
    end + 1,
    end + 2,
    replaced.offset
  );
  ...
  if(!node.attributes.find(a => a.name === 'width'))
    replaced = insert(
      replaced.content,
      ` width="${size.width}" `,
      end + 1,
    ...
    if(!node.attributes.find(a => a.name === 'height'))
      replaced = insert(
        replaced.content,
        ` height="${size.height}" `,
        end + 1,
  ...
  if (options.webp) {
    replaced = insert(
      replaced.content,
      getSrcset(sizes, srcsetLineWebp, "srcsetWebp").replaceAll('g/', '/g/'),
      end + 1,
      end + 2,
      replaced.offset
    );
